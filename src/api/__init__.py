...
class PredictionResponse(BaseModel):
    ...
    failure_type: str
    health_regime: str
    rul_hours: Optional[float]           # None if failure_prob < threshold
...
def engineer_features(inp: SensorInput) -> np.ndarray:
    ...
    high_wear_flag  = int(inp.tool_wear > 200)
    thermal_overload = int(temp_diff > 12)
...
def apply_pca(features: np.ndarray) -> np.ndarray:
    """Apply scaler + PCA if loaded, else pass through."""
    if "scaler" in MODELS:
        features = MODELS["scaler"].transform(features[:, :5])  # only raw 5 sensors
    if "pca" in MODELS:
        return MODELS["pca"].transform(features)
    return features
...
def predict_failure(features: np.ndarray, cluster: int) -> tuple:
    ...
    feat_with_cluster = np.hstack([features, [[cluster]]])
...
def estimate_rul(features: np.ndarray, fail_prob: float) -> Optional[float]:
    """Return estimated RUL hours, or None if below threshold."""
    if fail_prob < FAILURE_THRESHOLD:
        return None
    if "xgb_reg" not in MODELS:
        # Demo fallback
        rul = max(0, 500 - features[0, 4] * 2.5)
        return round(float(rul), 1)
    rul = float(MODELS["xgb_reg"].predict(features)[0])
    return round(max(0, rul), 1)
...
def predict(sensor: SensorInput, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        fail_prob, failure_type = predict_failure(features, cluster)

        # 5. Regression (conditional)
        rul = estimate_rul(features, fail_prob)
        ...
        logger.info(f"[PREDICT] {sensor.machine_id} → {failure_type} (p={fail_prob:.3f}, RUL={rul}h)")
        return response
