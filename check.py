...
class ModelPredictionError(RuntimeError):
    """Raised when a loaded model cannot complete inference."""
...
def _load_xgboost_model(name: str, path: str) -> Any:
    """Load an XGBoost model saved with the native JSON format."""
    try:
        import xgboost as xgb
    except ImportError as exc:
        raise RuntimeError(
            f"xgboost is required to load {path}. Install the project requirements in the API runtime."
        ) from exc
...
def _xgb_aligned_frame(model: Any, model_input: pd.DataFrame) -> pd.DataFrame:
    """Align input columns to names saved inside an XGBoost model, when present."""
    ...
    if expected_cols is not None:
        return model_input.reindex(columns=list(expected_cols), fill_value=0.0)
    return model_input


def _xgb_dmatrix(model: Any, model_input: pd.DataFrame):
    """Build a DMatrix that respects persisted booster feature names."""
    import xgboost as xgb

    aligned_input = _xgb_aligned_frame(model, model_input)
    booster_features = getattr(model.get_booster(), "feature_names", None)
    if booster_features:
        return xgb.DMatrix(aligned_input.to_numpy(), feature_names=list(booster_features))
    return xgb.DMatrix(aligned_input.to_numpy())


def _safe_classifier_proba(model: Any, model_input: pd.DataFrame) -> np.ndarray:
    """Return classifier probabilities for native-loaded or pickled wrappers."""
    try:
        if hasattr(model, "get_booster"):
            booster = model.get_booster()
            preds = booster.predict(_xgb_dmatrix(model, model_input))
        else:
            preds = model.predict_proba(model_input)
    except Exception as exc:
        raise ModelPredictionError("classification_probability", str(exc)) from exc
...
def _safe_classifier_predict(model: Any, model_input: pd.DataFrame) -> np.ndarray:
    """Return classifier predictions for native-loaded or pickled wrappers."""
    try:
        if hasattr(model, "get_booster"):
            proba = _safe_classifier_proba(model, model_input)
            if proba.ndim == 1:
        ...
        raise ModelPredictionError("classification_prediction", str(exc)) from exc


def _safe_regressor_predict(model: Any, model_input: pd.DataFrame) -> np.ndarray:
    """Return regressor predictions for native-loaded or pickled wrappers."""
    try:
        if hasattr(model, "get_booster"):
            booster = model.get_booster()
            return booster.predict(_xgb_dmatrix(model, model_input))
        return model.predict(model_input)
    except Exception as exc:
        raise ModelPredictionError("regression_prediction", str(exc)) from exc
...
def predict_failure(features: np.ndarray, cluster: int, pca_features: np.ndarray) -> tuple:
    ...
    stage1_model = MODELS.get("xgb_is_failure")
    stage2_model = MODELS.get("xgb_failure_types")
    multiclass_model = MODELS.get("xgb_clf")
    failed_classifiers = [
        name for name in ("xgb_is_failure", "xgb_failure_types", "xgb_clf")
        if name in MODEL_LOAD_ERRORS
    ]
    if failed_classifiers:
        details = "; ".join(f"{name}={MODEL_LOAD_ERRORS[name]}" for name in failed_classifiers)
        raise ModelPredictionError("model_loading", details)

    if stage1_model is None and multiclass_model is None:
        # Demo fallback when no classifier artefact is present in dev mode.
    ...
    if stage1_model is not None and stage2_model is not None:
        stage1_cols = getattr(stage1_model, "feature_names_in_", None)
        stage1_df = feat_df.reindex(columns=list(stage1_cols), fill_value=0.0) if stage1_cols is not None else feat_df
        stage1_proba = _safe_classifier_proba(stage1_model, stage1_df)[0]
        fail_prob = float(stage1_proba[-1]) if len(stage1_proba) > 1 else float(stage1_proba[0])
        if fail_prob < FAILURE_THRESHOLD:
            return fail_prob, "No Failure"

        stage2_cols = getattr(stage2_model, "feature_names_in_", None)
        stage2_df = feat_df.reindex(columns=list(stage2_cols), fill_value=0.0) if stage2_cols is not None else feat_df
        stage2_pred = _safe_classifier_predict(stage2_model, stage2_df)
        pred_class = int(stage2_pred[0]) if hasattr(stage2_pred, "__len__") else int(stage2_pred)
        if pred_class == 0:
            pred_class = 2
    ...
    if model is None:
        raise ModelPredictionError(
            ...
            "Loaded xgb_isFailure without xgb_failureTypes, and no xgb_classifier fallback is available.",
        )
    expected_cols = getattr(model, "feature_names_in_", None)
    model_df = feat_df.reindex(columns=list(expected_cols), fill_value=0.0) if expected_cols is not None else feat_df
    proba = _safe_classifier_proba(model, model_df)[0]
...
def estimate_tool_wear(features: np.ndarray, fail_prob: float) -> Optional[float]:
    ...
    if fail_prob < FAILURE_THRESHOLD:
        return None
    if "xgb_reg" in MODEL_LOAD_ERRORS:
        raise ModelPredictionError("model_loading", f"xgb_reg={MODEL_LOAD_ERRORS['xgb_reg']}")
...
def predict(sensor: SensorInput, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        detail = {
            "message": "Prediction failed while running the ML pipeline.",
            "error": str(e),
        }
